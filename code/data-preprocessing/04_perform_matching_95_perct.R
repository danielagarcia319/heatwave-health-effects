# Perform matching on 95th percentile data
...
candidate_dates <- heat_index_final %>% 
  filter(within_3d_hw_ind_95_2d == 0) %>%
  select(fips, date, ind) %>%
  mutate(doy = yday(date),
         year = year(date)) %>%
  as.data.table()
...
orig_inds_95 <- heatwave_dates$ind
hw_fips <- unique(heatwave_dates$fips)
matched_inds_95_df <- setNames(data.frame(matrix(ncol = 2, nrow = n)), c("fips", "matched_ind")) %>% 
  mutate(fips = as.integer(fips), matched_ind = as.integer(matched_ind)) %>%
  as.data.table()

# Run this code if paused running
#load("heatwave-health-effects/data/matching_95th_perct/matched_inds_95_df.Rdata")
...
for (j in last_j:length(hw_fips)) { 
  ...
  for(k in last_k:n_temp) {
    ...
    matched_inds_filter <- matched_inds_95_df[fips == fips_temp, matched_ind]
    ...
    # store results
    matched_inds_95_df[i, c("fips")] <- fips_temp
    matched_inds_95_df[i, c("matched_ind")] <- random_match
...
matched_inds_95 <- matched_inds_95_df[,matched_ind]
...
length(matched_inds_95[which(matched_inds_95 != 0)]) == length(unique(matched_inds_95[which(matched_inds_95 != 0)]))

# Display heatwave dates and their matched dates side-by-side
matching_results <- heatwave_dates %>%
  select(fips, date) %>%
  cbind(data.frame(matched_ind = matched_inds_95)) %>%
  left_join(candidate_dates[ind %in% matched_inds_95, c("ind", "date", "fips")] %>% 
...
all_inds_95 <- c(orig_inds_95[which(matched_inds_95 != 0)], 
                 matched_inds_95[which(matched_inds_95 != 0)])

# save results
save(matched_inds_95_df, file = "heatwave-health-effects/data/matching_95th_perct/matched_inds_95_df.Rdata")
save(matched_inds_95, file = "heatwave-health-effects/data/matching_95th_perct/matched_inds_95.Rdata")
save(all_inds_95, file = "heatwave-health-effects/data/matching_95th_perct/all_inds_95.Rdata")
