...
load("heatwave-health-effects/data/matching_97th_perct/all_inds_97.Rdata")

# Merge data for just diag1 diagnosis codes
matched_dataset_97_hosp <- heat_index_final %>%
  filter(ind %in% all_inds_97) %>%
  mutate(year = year(date)) %>%
  left_join(hosp_agg, by = c("fips", "date")) %>% 
  replace(is.na(.), 0)

# Shows results
matched_dataset_97_hosp %>% 
...
save(matched_dataset_97_hosp, file = "heatwave-health-effects/data/matched_dataset_97_hosp.Rdata")
